	  ...
	  if (current_ir_type () != IR_GIMPLE || currently_expanding_to_rtl)
	    {
	      ...
	      /* stack_protect_prologue call us too early for
	         entry_of_function to work properly.  Need to check
		 this condition here. */
	      if (ENTRY_BLOCK_PTR->next_bb->flags & BB_RTL)
		entry = entry_of_function ();
	      else
		{
