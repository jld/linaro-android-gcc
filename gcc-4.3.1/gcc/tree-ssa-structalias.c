...
/* ANDROID LOCAL BEGIN */
/* Additional comparisons to make qsort stable.  */
  if (foasize != fobsize)
    return foasize - fobsize;

  foasize = foa->decl ? DECL_UID (foa->decl) : 0;
  fobsize = fob->decl ? DECL_UID (fob->decl) : 0;
  if (foasize != fobsize)
    return foasize - fobsize;

  foasize = TYPE_UID (foa->type);
  fobsize = TYPE_UID (fob->type);
/* ANDROID LOCAL END */
  return foasize - fobsize;
}
