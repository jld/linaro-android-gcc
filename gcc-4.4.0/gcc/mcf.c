...
/* Compute the lineno of the last stmt in BB. */

static int
compute_bb_lineno_last (basic_block bb)
{
  int lineno = 0;
...
/* Check if the source and dest of an edge are on the same source line. */

static bool
same_line_with_preds (VEC (edge, gc) *from_edges)
{
  edge e;
  edge_iterator ei;
  FOR_EACH_EDGE (e, ei, from_edges)
    {
      if (compute_bb_lineno_last (e->src) == compute_bb_lineno_last (e->dest))
        return true;
    }
  return false;
}
