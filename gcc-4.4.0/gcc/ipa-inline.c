...
cgraph_edge_badness (struct cgraph_edge *edge)
{
  ...
  if (growth <= 0)
    {
      ...
      if (dump_file)
	fprintf (dump_file, "Growth is less than zero.\n");
    }
  ...
  /* When profiling is available, base priorities -(#calls / growth).
  ...
  else if (max_count)
    {
      if (flag_sample_profile
          && get_total_count (edge->call_stmt,
                              cgraph_node_name (edge->caller)) > 0)
        {
          ...
          if (dump_file)
            fprintf (dump_file,
                     "Sample profiling and edge has non-zero sample count.\n");
        }
      else
        {
          badness = ((int)((double)edge->count * INT_MIN / max_count))
              / growth;
          if (dump_file)
            fprintf (dump_file, "Profiling, edge count = %d (%d)\n",
                     (int) edge->count,
                     ((int)((double)edge->count * INT_MIN / max_count)));
        }
    }
  ...
  /* When function local profile is available, base priorities on
  ...
  else if (flag_guess_branch_prob)
    {
      ...
      int all_growth = cgraph_estimate_growth (edge->callee);
      int growth =
	cgraph_estimate_size_after_inlining (1, edge->caller, edge->callee);
      growth -= edge->caller->global.insns;
      badness = growth * 256;
      ...
      if (dump_file)
	fprintf (dump_file,
                 "Guessing probability, growth = %d, div = %d, "
                 "growth inlining all calls = %d\n",
		 growth, div, all_growth);
  ...
  /* When function local profile is not available or it does not give
  ...
  else
    {
      ...
      int all_growth = cgraph_estimate_growth (edge->callee);
      badness = all_growth * 256;

      if (dump_file)
	fprintf (dump_file,
                 "No profile, growth inlining all callees = %d, nest = %d\n",
		 all_growth, nest);
  ...
  if (cgraph_recursive_inlining_p (edge->caller, edge->callee, NULL))
    {
      if (dump_file)
	fprintf (dump_file, "Recursive inlining, increment badness.\n");
      badness++;
    }
  ...
  if (dump_file)
    fprintf (dump_file, "Badness for (%s/%d -> %s/%d) = %d\n",
	...
	if (edge->aux)
	  {
	    ...
	    if (dump_file)
	      fprintf (dump_file,
                       "Edge %s/%d -> %s/%d badness changed to %d from %d\n",
		       cgraph_node_name (edge->caller), edge->caller->uid,
		       cgraph_node_name (edge->callee), edge->callee->uid,
		       badness, (int)n->key);
	    ...
	    if (fibheap_replace_key (heap, n, badness))
	      continue;
              ...
              {
                fprintf (dump_file,
                         "HEAP_ADD late (%s/%d -> %s/%d)/%d, badness %d\n",
                         cgraph_node_name (edge->caller), edge->caller->uid,
                         cgraph_node_name (edge->callee), edge->callee->uid,
                         edge->uid, badness);
                fprintf (dump_file, "END HEAP_ADD added to heap\n");
              }
...
cgraph_decide_inlining_of_small_functions (void)
{
  ...
  for (node = cgraph_nodes; node; node = node->next)
	...
	{
	  int nhot = 0;
	  tree decl = (node->inline_decl ? node->inline_decl : node->decl);
	  for (edge = node->callers; edge; edge = edge->next_caller)
            if (edge->inline_failed && cgraph_maybe_hot_edge_p (edge))
              nhot++;
	  fprintf (dump_file,
                   "FUNCTION %s/%d, %d insns, %s, hot edges %d, %s\n",
		   cgraph_node_name (node), node->uid, node->global.insns,
		   (DECL_DECLARED_INLINE_P (decl) ?
                    "declared inline" : "not declared inline"),
		   nhot, function_frequency_string (node));
	}

      if (!node->callers)
	{
	  if (dump_file)
	    fprintf (dump_file, "END FUNCTION no callers.\n");
	  continue;
	}
	...
	{
	  if (dump_file)
	    fprintf (dump_file, "END FUNCTION not inlinable.\n");
	  continue;
	}
	...
	{
	  if (dump_file)
	    fprintf (dump_file, "END FUNCTION not candidate: %s\n",
                     failed_reason);
      ...
      if (dump_file)
	fprintf (dump_file, "END FUNCTION inline candidate.\n");

      for (edge = node->callers; edge; edge = edge->next_caller)
	{
	  int badness = cgraph_edge_badness (edge);
	  if (dump_file)
	    fprintf (dump_file, "HEAP_ADD (%s/%d -> %s/%d)/%d, badness %d\n",
		     cgraph_node_name (edge->caller), edge->caller->uid,
		     cgraph_node_name (edge->callee), edge->callee->uid,
		     edge->uid, badness);
	  ...
	  if (edge->inline_failed)
	  ...
	  else
	    {
	      if (dump_file)
		fprintf (dump_file,
                         "END HEAP_ADD not added inline_failed is NULL.\n");
	    }
	}
    }
  ...
  while (overall_insns <= max_insns
	...
	cgraph_estimate_size_after_inlining (1, edge->caller, edge->callee);
      ...
      if (dump_file)
        fprintf (dump_file, "EDGE (%s/%d -> %s/%d)/%d, growth %d, "
                 "badness %d, frequency %f (%d), hot %d, "
                 "caller frequency %s, callee frequency %s\n",
                 cgraph_node_name (edge->caller), edge->caller->uid,
                 cgraph_node_name (edge->callee), edge->callee->uid,
                 edge->uid,
                 cgraph_estimate_growth (edge->callee),
                 cgraph_edge_badness (edge),
                 edge->frequency / (double)CGRAPH_FREQ_BASE, (int) edge->count,
                 cgraph_maybe_hot_edge_p (edge),
                 function_frequency_string (edge->caller),
                 function_frequency_string (edge->callee));
	...
	{
	  if (dump_file)
	      fprintf (dump_file, "END EDGE inline_failed non-NULL.\n");
	  continue;
	}
      ...
      if (!edge->count)
	{
	  ...
	  if (where->global.inlined_to)
		...
		= (edge->callee->local.disregard_inline_limits ? N_("recursive inlining") : "");
	      if (dump_file)
		fprintf (dump_file,
                         "END EDGE inline_failed: Recursive inlining performed "
                         "only for function itself.\n");
	      continue;
	    }
	}
      ...
      if (not_good && growth > 0 && cgraph_estimate_growth (edge->callee) > 0)
	{
				            ...
				            &edge->inline_failed))
	    {
	      edge->inline_failed = not_good;
              if (dump_file)
                fprintf (dump_file, "END EDGE inline_failed: %s.\n",
                         edge->inline_failed);
	    }
          else
            {
              if (dump_file)
                fprintf (dump_file,
                         "END EDGE inline_failed: code size growth and "
                         "is recursive.\n");
            }
	  continue;
	}
          ...
          && !better_inline_comdat_function_p (edge->callee))
	{
				            ...
				            &edge->inline_failed))
	    {
          ...
          else
            {
              if (dump_file)
                fprintf (dump_file,
                         "END EDGE inline_failed: fails default inlining "
                         "and is recursive.\n");
            }
	  continue;
	}
      ...
      else
	{
	  ...
	  if (gimple_call_cannot_inline_p (edge->call_stmt)
					      ...
					      &edge->inline_failed, true))
	    {
	      if (dump_file)
		fprintf (dump_file,
                         "END EDGE cannot inlining or limit failed: %s.\n",
			 edge->inline_failed);
	      continue;
	    }
          if (!dbg_cnt (inl))
  ...
  while ((edge = (struct cgraph_edge *) fibheap_extract_min (heap)) != NULL)
    {
      ...
      if (dump_file)
	{
	  fprintf (dump_file, "EDGE post-limit (%s/%d -> %s/%d)/%d, growth %d, "
                   "badness %d, frequency %f (%d), hot %d, "
                   "caller frequency %s, callee frequency %s\n",
		   cgraph_node_name (edge->caller), edge->caller->uid,
		   cgraph_node_name (edge->callee), edge->callee->uid,
		   edge->uid, cgraph_estimate_growth (edge->callee),
		   cgraph_edge_badness (edge),
                   (edge->frequency / (double)CGRAPH_FREQ_BASE),
		   (int) edge->count, cgraph_maybe_hot_edge_p (edge),
		   function_frequency_string (edge->caller),
		   function_frequency_string (edge->callee));
	}
				           ...
				           &edge->inline_failed))
	{
	  ...
	  if (dump_file)
	    {
	      fprintf (dump_file, "END EDGE %s\n", edge->inline_failed);
	      fprintf (dump_file,
		       "Limit reached: not inlining %s/%d -> %s/%d, "
                       "badness %d, frequency %f (%d)\n",
		       cgraph_node_name (edge->caller), edge->caller->uid,
		       cgraph_node_name (edge->callee), edge->callee->uid,
		       cgraph_edge_badness (edge),
                       (edge->frequency / (double)CGRAPH_FREQ_BASE),
      ...
      else
	{
	  if (dump_file)
	    fprintf (dump_file,
                     "END EDGE disregard_inline_limits or inlined_failed or "
                     "recursive_inlining_p\n");
	}
    }
  ...
  if (dump_file)
    {
      ...
      fprintf (dump_file, "max_count = %d\n", (int) max_count);
      fprintf (dump_file, "flag_inline_functions = %d\n",
               flag_inline_functions);
      fprintf (dump_file, "flag_guess_branch_prob = %d\n",
               flag_guess_branch_prob);
      fprintf (dump_file, "profile_info_available_p = %d\n",
               profile_info_available_p());
      ...
      for (i = nnodes - 1; i >= 0; i--)
	{
	  ...
	  if (node->callers
	      ...
	      else
		{
		  if (dump_file)
		    {
		      fprintf (dump_file, "END FUNCTION_ONCE not inlined\n");
		      fprintf (dump_file,
  ...
  if (e)
    {
      timevar_push (TV_INTEGRATION);
      todo = optimize_inline_calls (current_function_decl);
      /* Smooth the basic block count if there's sample profile for this
         function. */
