...
dnl
dnl Substs:
dnl  DETERMINISM_CXXFLAGS
dnl  OPTIMIZE_CXXFLAGS
dnl  WARN_FLAGS
dnl
AC_DEFUN([GLIBCXX_EXPORT_FLAGS], [
  # By default we use a fixed random seed per output.  (This is expanded
  # by make to set the random seed for each output file to be the output
  # file's name.)
  DETERMINISM_CXXFLAGS=-frandom-seed=\$[@]
  AC_SUBST(DETERMINISM_CXXFLAGS)
