...
/* Gimplify CALL_EXPR.  What are really want to do is to intercept all
   call to base ctors/dtors that can be replaced by calling corresponding
   ctors/dtors when there is no virtual inheritance. */

static int
gimplify_call_expr (tree *expr_p, tree *pre_p, tree *post_p)
{
  tree call_expr = *expr_p;
  tree call_target = TREE_OPERAND (call_expr, 0);
  tree decl = NULL;

  if (flag_declone)
    {
      if (TREE_CODE (call_target) == ADDR_EXPR &&
	  TREE_CODE (TREE_OPERAND (call_target, 0)) == FUNCTION_DECL)
	{
	  tree complete, new_addr_expr;

	  decl = TREE_OPERAND (call_target, 0);
	  if (decloned_base_cdtor_p (decl)) 
	    {
	      gcc_assert (!processing_template_decl);
	      complete = get_complete_for_base (decl);
	      gcc_assert (complete);

	      /* Replace call target to complete constructor/destructor. */
	      new_addr_expr = build1 (ADDR_EXPR, TREE_TYPE (call_target),
				      complete);
	      *expr_p = build3 (CALL_EXPR, TREE_TYPE (call_expr),
				new_addr_expr,
				TREE_OPERAND (call_expr, 1),
				TREE_OPERAND (call_expr, 2));
...
cp_gimplify_expr (tree *expr_p, tree *pre_p, tree *post_p)
{
  ...
  switch (code)
	...
	*expr_p = (TREE_TYPE (arg) != type) ? fold_convert (type, arg)
    ...
    case CALL_EXPR:
      ret = gimplify_call_expr (expr_p, pre_p, post_p);
      break;
