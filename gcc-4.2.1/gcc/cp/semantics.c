...
/* Assemble an alias for a complete ctor/dtor that can replace its
   base counterpart.  Returns true if successful.  */

static bool
maybe_skip_base_cdtor (tree base)
{
  tree complete, alias, attrs, name, args;

  /* If base is not visible from outside, we do not need to output anything
     as gimplfier already rewrites all calls to it.  */
  if (flag_syntax_only || !TREE_PUBLIC (base))
    return true;
    
  complete = get_complete_for_base (base);
  gcc_assert (complete && TREE_PUBLIC (complete));

  mangle_decl (base);
  mangle_decl (complete);
  alias = make_alias_for (complete, DECL_ASSEMBLER_NAME (base));

  /* Setting attributes so that alias gets the same linkage and visibility
     as complete ctor/dtor.  */
  DECL_EXTERNAL (alias) = DECL_EXTERNAL (complete);
  DECL_ONE_ONLY (alias) = DECL_ONE_ONLY (complete);
  DECL_VISIBILITY (alias) = DECL_VISIBILITY (complete);
  DECL_VISIBILITY_SPECIFIED (alias) = DECL_VISIBILITY_SPECIFIED (complete);
  DECL_WEAK (alias) = DECL_WEAK (complete);
  TREE_PUBLIC (alias) = TREE_PUBLIC (complete);

  DECL_INITIAL (alias) = NULL_TREE;

  /* Make this a weak alias if complete is weak */
  if (DECL_WEAK (complete))
    {
      /* Can this happen?  Better be safe than sorry.  */
      if (!SUPPORTS_WEAK)
	return false;

      name = build_string (IDENTIFIER_LENGTH (DECL_NAME (complete)),
			   IDENTIFIER_POINTER (DECL_NAME (complete)));
      args = tree_cons (NULL_TREE, name, NULL_TREE);
      attrs = tree_cons (get_identifier ("alias"), args, NULL);
      attrs = tree_cons (get_identifier ("weak"), NULL_TREE, attrs);

      decl_attributes (&alias, attrs, 0);
    }

  assemble_alias (alias, DECL_ASSEMBLER_NAME (complete));
  TREE_USED (complete) = true;
  return true;
}
...
expand_body (tree fn)
{
  ...
  /* If FN is a base ctor or dtor that can be replace by its complete
     counterpart,  check to see if we can skip outputting the function
     body.  */
  if (flag_declone && decloned_base_cdtor_p (fn))
    {
      gcc_assert (!processing_template_decl);

      if (maybe_skip_base_cdtor (fn))
	{
	  TREE_ASM_WRITTEN (fn) = true;
	  return;
	}
    }
