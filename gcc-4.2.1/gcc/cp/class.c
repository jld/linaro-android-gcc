...
/* Returns true iff we can avoid making the base clone of a class type t.
   Caller must check that it is not in the middle of template processing.
   In general, it is not possible to tell if a template class has any
   virtual bases until it is fully instantiated.  */

bool
disable_cdtor_cloning_p (tree t)
{
  return !(CLASSTYPE_VBASECLASSES (t));
}

/* Returns true iff this base ctor/dtor can be replaced by its complete
   counterpart.  Caller must check for invalidity due to templates. */

bool
decloned_base_cdtor_p (tree decl)
{
  tree name = DECL_NAME (decl);
  return ((name == base_ctor_identifier || name == base_dtor_identifier)
	  && DECL_CONTEXT (decl)
	  && disable_cdtor_cloning_p (DECL_CONTEXT (decl)));
}

/* Returns true iff this complete ctor/dtor can replace its base
   counterpart.  Caller must check for invalidity due to templates. */

bool
decloned_complete_cdtor_p (tree decl)
{
  tree name = DECL_NAME (decl);
  return ((name == complete_ctor_identifier || name == complete_dtor_identifier)
	  && DECL_CONTEXT (decl)
	  && disable_cdtor_cloning_p (DECL_CONTEXT (decl)));
}

/* For a base constructor or destructor, find its corresponding complete
   counterpart.  Returns NULL_TREE if none is found.  */

tree
get_complete_for_base (tree base)
{
  tree source, clone, name;

  if (DECL_NAME (base) == base_ctor_identifier)
    name = complete_ctor_identifier;
  else if (DECL_NAME (base) == base_dtor_identifier)
    name = complete_dtor_identifier;
  else
    gcc_unreachable ();

  /* A base ctor/dtor must be a clone of a may-be-in-charge ctor/dtor. */ 
  source = DECL_CLONED_FUNCTION (base);
  gcc_assert (source);

  FOR_EACH_CLONE (clone, source)
    {
...
#if 0
  /* Too many false positives -- spark@google.com, mec@google.com.  */
